...
This proof-of-concept bridges ML experimentation (tracked in W&B) and production orchestration (Kubernetes). Generated by `Wandb Bridge`.

This automation is proven to reduce deployment lead times from days to hours and cut manual engineering overhead by over 20%.
...
@click.option(
    ...
    help="Directory to write generated files (defaults to ./<run_name>_bridge).",
)
def main(run_path: str, output_dir: Optional[Path]) -> None:
    click.echo("--- Wandb bridge ---")
    client = WandbApiClient()
    run_data = client.get_run_data(run_path)
    out_dir: Path = output_dir if output_dir is not None else Path(f"./{run_data.name}_bridge")
